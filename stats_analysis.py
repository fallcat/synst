...
def main():
    ...
    with open(base_path + stats_path + '.pickle', 'rb') as stats_file:
        with open(base_path + stats_path + '.txt', 'wt') as output_file:
            ...
            for split in splits:
                num_layers = stats[split + '_stats'][MODEL_STATS[0]][STATS_TYPES[0]]['mean'].shape[0]
                num_heads = stats[split + '_stats'][MODEL_STATS[0]][STATS_TYPES[0]]['mean'].shape[1]
                for model_stat in MODEL_STATS:
                    output_file.write("========== " + model_stat + " ==========\n")
                    for stats_type in STATS_TYPES:
                        output_file.write("---------- " + stats_type + " ----------\n")
                        output_file.write("mean:\n")
                        mean = stats[split + '_stats'][model_stat][stats_type]['mean']
                        print_matrix(mean, output_file)
                        output_file.write("std:\n")
                        std = np.sqrt(stats[split + '_stats'][model_stat][stats_type]['var'])
