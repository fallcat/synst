#!/bin/bash
#
#SBATCH --job-name=iwslt_grid_0308
...
#SBATCH -o /mnt/nfs/work1/miyyer/simengsun/synst/experiments/iwslt_grid_0308/output_train.txt
#SBATCH --mail-type=ALL
#SBATCH --mail-user=simengsun@cs.umass.edu
BASE_PATH=/mnt/nfs/work1/miyyer
PROJECT_PATH=$BASE_PATH/simengsun/synst
EXPERIMENT_PATH=$PROJECT_PATH/experiments/iwslt_grid_0308

		
right	right	center	right	left	left	center	center	
...
env $(cat ~/.comet.ml | xargs) python main.py \
  ...
  --attn-param 1 \
  --attn-type normal \
  --attn-position right right left right left right left right center right \
  --attn-displacement 1 \
  --dec-attn-param 1 \
